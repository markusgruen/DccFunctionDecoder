...
namespace OutputController{
  ...
  /**
   * @brief Non-blocking wait function - waits (10 ms * waitTime);
  ...
  bool forwardOnly(uint8_t channel) {
    // return bit_is_set(functionMap[channel], 22);  // bit_is_set only works up to the 7th bit !!!
    return ((functionMap[channel]) & (1UL << (22)));

  }
  
  /// @brief Return true if channel is reverse-only.
  bool reverseOnly(uint8_t channel) {
    return ((functionMap[channel]) & (1UL << (23)));
  }
