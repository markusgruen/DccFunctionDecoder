...
void DccSignalDecoder::addBitsToBitstream() {
  ...
  // copy the first "numNewBits" of "newBitstream" to position "mNumBits" of mBitstream
  // mBitstream |= (  newBitstream & ((1ULL << numNewBits)-1)  ) << mNumBits;
  // ^ funktioniert, aber so ists einfacher:
...
void DccSignalDecoder::TESTsetBitstream(uint64_t bitstream, uint8_t numValidBits) {
  mBitstream = bitstream;
  mNumBits = numValidBits;
}
