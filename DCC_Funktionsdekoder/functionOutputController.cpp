...
void FunctionOutputController::begin(uint8_t pin1, uint8_t pin2, uint8_t pin3, uint8_t pin4){
  ...
  mChannelPin_bm[0] = digitalPinToBitMask(pin1);
  mChannelPin_bm[1] = digitalPinToBitMask(pin2);
  mChannelPin_bm[2] = digitalPinToBitMask(pin3);
  mChannelPin_bm[3] = digitalPinToBitMask(pin4);
...
void FunctionOutputController::update(Direction direction, uint8_t speed, uint32_t functions) {
  for(uint8_t channel=0; channel<NUM_CHANNELS; channel++) {
    //bool channelWasOn = wasChannelOn(functions, channel);
    //bool channelIsOn  = isChannelSwitchedOn(functions, channel);
